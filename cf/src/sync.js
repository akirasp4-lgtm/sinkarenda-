...
// ★2026-09-03 夜勤区分: 夜勤手当・夜勤請求 を追加（GAS の HEADERS と同じ順番）。
//   ここに足さないと、GASが23列を返した瞬間に取り込みが止まる
//   （「22列目は想定外の列です」で全件が同期されず、画面はGAS直読みへ落ちて遅くなる）。
//   ★反映の順番は Worker → GAS → 画面。これを守らないと無用な障害になる。
export const OPTIONAL_HEADERS = ['拠点', '部隊', '夜勤手当', '夜勤請求'];
