...
describe('重複の判定（設計書§1.1のルール）', () => {
  ...
  it('★グローライズとGRミツマは1つの名簿として見る（2026-08-27 実機で発見）', () => {
    // §3.9 でこの2社は1つの名簿に統合した。同じ人が本社案件（グローライズ）と
    // 関東案件（GRミツマ）の両方に登録されるので、会社で分けると
    // 「応援に出ている人の重複」を丸ごと見逃す。
    // ★実測: 会社で分けると39件/今日以降11件、1つの名簿として見ると47件/12件。
    //   差の8件はすべて本社⇔関東をまたぐ本物の重複だった。
    expect(P.findConflicts([
      row({ name: '高田（関東）', company: 'グローライズ', loc: 'A現場' }),
      row({ name: '高田（関東）', company: 'GRミツマ', loc: 'B現場' })
    ]).length).toBe(1);
  });

  it('他事業（和信カインド・ラーテル・GRHD）は別の名簿のまま', () => {
    ['和信カインド', 'ラーテル', 'GRHD'].forEach(co => {
      expect(P.findConflicts([
        row({ name: '元', company: co, loc: 'A現場' }),
        row({ name: '元', company: 'グローライズ', loc: 'B現場' })
      ]).length, co).toBe(0);
    });
  });
